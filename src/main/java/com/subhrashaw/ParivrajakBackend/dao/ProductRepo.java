...
public interface ProductRepo extends JpaRepository<Product,Integer> {
    ...
    @Query("SELECT p FROM Product p WHERE p.price BETWEEN :startPrice AND :endPrice AND p.minDays >= :startDay AND p.maxDays <= :endDay")
    ...
    List<Product> findByPrice(@Param("startPrice") double startPrice,
                              @Param("endPrice") double endPrice);
    @Query("SELECT p FROM Product p WHERE p.minDays >= :startDay AND p.maxDays <= :endDay")
